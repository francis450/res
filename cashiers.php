...
<html>
<head>
  <title><?php echo $userid;?> order list</title>
  ...
  $(document).ready(function(){
	  ...
	  $("#ref").keyup(function(){
	   ...
	   $(".menu-btn").click(function () {
		...
		$('.del').click(function(){
		 var delthisorder = $(this).attr('foo');
		   $(this).closest('tr').hide();
			$.post('register.php',{delthisorder:delthisorder},function(data){
					$('#response').fadeIn().html(data);
...
<body>
  <div class="container">
    ...
    <h3>Welcome <?php echo $userid;?>,this is your orders list</h3>
    <div class="tabs">
	<div class="tab tab-link" id="login-tab" onclick="openTab(event, 'overview')">Pedding orders</div>
	<div class="tab tab-link" id="login-tab" onclick="openTab(event, 'kitcheninventory')">On progress</div>
      <div class="tab tab-link" id="login-tab" onclick="openTab(event, 'login')">Ready</div>
	...
	<div id="overview" class="tabcontent">
        <table class="table stripe" style="border: 1px solid black;">
		  ...
		  <tbody>
			  <?php
			  $cashier = $_GET['r'];
			  $getmenut = mysqli_query($con,"select * from peddingorders where waiter='$cashier' and status='0'");
			  while($getsuppliersr = mysqli_fetch_array($getmenut)){
				 ...
				 if($status=='0'){
					 echo '<td><button fooda="'.$getsuppliersr['ordernumber'].'" class="btn qt" style="background-color:red" value="'.$getsuppliersr['status'].'">Pedding</button></td>';
				 }else if($status=='1'){
					 echo '<td><button fooda="'.$getsuppliersr['ordernumber'].'" class="btn rt" style="background-color:grey" value="'.$getsuppliersr['status'].'">On progress</button></td>';
				 }else if($status=='2'){
					 echo '<td><button fooda="'.$getsuppliersr['ordernumber'].'" class="btn st" style="background-color:blue" value="'.$getsuppliersr['status'].'">Ready</button></td>';
				 }else if($status=='3'){
					 echo '<td><button fooda="'.$getsuppliersr['ordernumber'].'" class="btn pt" style="background-color:green" value="'.$getsuppliersr['status'].'">pay</button></td>';
				 }
	...
	<div id="kitcheninventory" class="tabcontent">
         <table class="table stripe" style="border: 1px solid black;">
		  ...
		  <tbody>
			  <?php
			  $cashier = $_GET['r'];
			  $getmenut = mysqli_query($con,"select * from peddingorders where waiter='$cashier' and status='1'");
			  while($getsuppliersr = mysqli_fetch_array($getmenut)){
				  echo '<tr>';
				  echo '<td>'.$getsuppliersr['id'].'</td>';
				  echo '<td>'.$getsuppliersr['ordernumber'].'</td>';
    ...
    <div id="login" class="tabcontent">
      <table class="table stripe" style="border: 1px solid black;">
		  ...
		  <tbody>
			  <?php
			  $cashier = $_GET['r'];
			  $getmenut = mysqli_query($con,"select * from peddingorders where waiter='$cashier' and status='2'");
			  while($getsuppliersr = mysqli_fetch_array($getmenut)){
				  echo '<tr>';
				  echo '<td>'.$getsuppliersr['id'].'</td>';
				  echo '<td>'.$getsuppliersr['ordernumber'].'</td>';
    ...
    <div id="register" class="tabcontent">
     <table class="table stripe" style="border: 1px solid black;">
		  ...
		  <tbody>
			  <?php
			  $getmenut = mysqli_query($con,"select * from peddingorders where waiter='$cashier' and status='4'");
			  while($getsuppliersr = mysqli_fetch_array($getmenut)){
				  echo '<tr>';
				  echo '<td>'.$getsuppliersr['id'].'</td>';
				  echo '<td>'.$getsuppliersr['ordernumber'].'</td>';
				 ...
				 $now = date("Y-m-d H:i:s");
				  ...
				  //echo '<td bill ="'.$getsuppliersr['ordernumber'].'" class="del"><a href="pay.php?bill='.$getsuppliersr['ordernumber'].'&amount='.$amount.'">Pedding payment</a></td>';
				  echo '<td bill ="'.$getsuppliersr['ordernumber'].'" class="del"><a>Pedding payment</a></td>';
			  }
			  ?>
      ...
      <table class="table stripe" style="border: 1px solid black;">
		  ...
		  <tbody>
			  <?php
			  $getmenut = mysqli_query($con,"select * from peddingorders where waiter='$cashier' and status='5'");
			  while($getsuppliersr = mysqli_fetch_array($getmenut)){
				  echo '<tr>';
				  echo '<td>'.$getsuppliersr['id'].'</td>';
				  echo '<td>'.$getsuppliersr['ordernumber'].'</td>';
				 ...
				 $now = date("Y-m-d H:i:s");
				  ...
				  //echo '<td bill ="'.$getsuppliersr['ordernumber'].'" class="del"><a href="pay.php?bill='.$getsuppliersr['ordernumber'].'&amount='.$amount.'">Pedding payment</a></td>';
				  echo '<td bill ="'.$getsuppliersr['ordernumber'].'" class="del"><a>Paid</a></td>';
			  }
			  ?>
